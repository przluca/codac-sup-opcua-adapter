...
<!--
########################################################################
# $HeadURL: https://svnpub.iter.org/codac/iter/codac/dev/units/m-sup-adapters/tags/CODAC-CORE-6.2B2/src/main/resources/xmi2snl_adapter002.xsl $
...
Script can be run as (example): java -jar /opt/codac/lib/saxon-9.1.0.8.jar -xsl:xmi2snl_adaptor001.xsl -s:auto1.xml adapter=KSTAR_COSAdapter
-->

<!--
    TODO: Don't automatically generate dummy parent state, if there is no transition back to the parent state-machine.
-->
<xsl:stylesheet version="2.0" xmlns="http://www.w3.org/2005/07/scxml" 
    ...
    <xsl:variable name="adapter_element" select='//packagedElement[@xmi:type="uml:Component" and @name=$adapter]'/>
    ...
    <!-- ###################### DEFINE PART ###################### -->
    <xsl:template match='/'>
        <xsl:value-of select='concat("Adapter: ", $adapter, " ")'/>
...
<!-- ################### END OF DEFINE PART #################  -->
    ...
    <xsl:template match="region/subvertex">
        ...
        <!-- TODO: These are hard-coded right now. Should fetch names, if any, from model. Name may be undefined -->
        <!-- Quick hack, look for PV definition -->
        <xsl:variable name="Control_State" select='packagedElement[@xmi:type="uml:Class" and @name="Control_State"]'/>
        <xsl:text>&#xa;        entry {&#xa;</xsl:text>
        <xsl:choose>
            <xsl:when test = "$OPStateName = $top_state">
                <xsl:if test="$Control_State != ''">
                    <!-- (untested) PV Name is: $Control_State/ownedAttribute[@name="PV_Name"]/defaultValue/@value --> 
    ...
    <xsl:template mode="toparent" match="transition">
        ...
        <xsl:if test="$target_name_state != ''">
            ...
            <!--- TODO Temporary: Does not work right now
            <xsl:text>&#xa;    //  Psuedo-state that matches the parent state-machine state&#xa;</xsl:text>
            <xsl:text>    state </xsl:text>
            <xsl:value-of select="$target_name_state"/>
            <xsl:text> { </xsl:text>
            <!-X- Find transitions back to the child state machine and proceed to the hidden state -X->

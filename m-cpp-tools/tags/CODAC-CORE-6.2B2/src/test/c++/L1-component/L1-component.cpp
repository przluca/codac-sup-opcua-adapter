/******************************************************************************
* $HeadURL: https://svnpub.iter.org/codac/iter/codac/dev/units/m-cpp-tools/tags/CODAC-CORE-6.2B2/src/test/c++/L1-component/L1-component.cpp $
* $Id: L1-component.cpp 96477 2019-01-09 14:53:55Z sivecm $
*
  ...
  for (ccs::types::uint32 index = 0u; (status && (index < __assoc.size())); index += 1u)
    {
      ...
      status = (__ca_clnt->AddVariable(std::get<1>(__assoc[index]).c_str(), ccs::types::AnyputVariable, std::get<2>(__assoc[index])) &&
		__ca_clnt->SetCallback(std::get<1>(__assoc[index]).c_str(), std::bind(&RPCServiceImpl::HandleCAMonitor, this, _1, _2))); // Install CA monitor callback
    }
...
bool RPCServiceImpl::Connect (const char* service)
{
  ...
  ccs::types::AnyValue request (ccs::base::PVAccessRPC::Request_int);

  if (status)
    {
      // Snapshot request
      status = ccs::HelperTools::SetAttributeValue(&request, "qualifier", "echo");
    }

  if (status)
    {
      // Dummy request
      __rpc_clnt->SendRequest(request);
    }
...
ccs::types::AnyValue RPCServiceImpl::HandleRequest (const ccs::types::AnyValue& request)
{
  ...
  if (qualifier == "connect") // Connect to named L0 component
    {
      std::string l0_service;
      ...
      if (status)
	{
	  status = this->Connect(l0_service.c_str());
	}
      ...
      if (status)
	{
	  snprintf(reply.reason, STRING_MAX_LENGTH, "Connected to '%s' RPC service", l0_service.c_str());
	}
      else
	{
	  ccs::HelperTools::SafeStringCopy(reply.qualifier, "error", STRING_MAX_LENGTH);
	  snprintf(reply.reason, STRING_MAX_LENGTH, "Error with client to '%s' RPC service", l0_service.c_str());
	}
  ...
  if (qualifier == "read")
      ...
      for (ccs::types::uint32 index = 0u; (status && (index < __assoc.size())); index += 1u)
	{
	  ...
	  if (status)
	    {
	      // Update cache
	      log_info("RPCServiceImpl::HandleRequest('%s') - Initialise cache '%s' attribute", qualifier.c_str(), std::get<0>(__assoc[index]).c_str());
	      memcpy(ccs::HelperTools::GetAttributeReference(__config_cache, std::get<0>(__assoc[index]).c_str()), 
		     ccs::base::ChannelAccessInterface::GetInstance<ccs::base::ChannelAccessClient>()->GetVariable(std::get<1>(__assoc[index]).c_str())->GetInstance(), 
		     ccs::HelperTools::GetAttributeType(__config_cache, std::get<0>(__assoc[index]).c_str())->GetSize());
	    }
	  else
	    {
  ...
  if (qualifier == "load")
    {
      // Seed and hash from the RPC message .. config retrieved from the cache (updated through CA monitor)
      ...
      request_type.AddAttribute("value", __config_struct);
      request_type.AddAttribute<ccs::types::uint32>("hash");

      ccs::types::AnyValue l0_request (request_type);

      char buffer [1024]; __config_cache->SerialiseInstance(buffer, 1024u);
      log_info("RPCServiceImpl::HandleRequest('%s') - Read configuration from cache '%s'", qualifier.c_str(), buffer);

      ccs::HelperTools::SetAttributeValue(&l0_request, "qualifier", "load");
      ccs::HelperTools::SetAttributeValue(&l0_request, "seed", seed);
      ccs::HelperTools::SetAttributeByReference(&l0_request, "value", __config_cache->GetInstance());
      ccs::HelperTools::SetAttributeValue(&l0_request, "hash", hash);

      log_info("RPCServiceImpl::HandleRequest('%s') - Query from '%s' RPC service  ..", qualifier.c_str(), __rpc_clnt->GetService());
      l0_request.SerialiseInstance(buffer, 1024u);
      log_info("RPCServiceImpl::HandleRequest('%s') - .. with request '%s'", qualifier.c_str(), buffer);
      return __rpc_clnt->SendRequest(l0_request);
    }
...
void RPCServiceImpl::HandleCAMonitor (const char* name, const ccs::types::AnyValue& request)
{
  ...
  bool status = (static_cast<ccs::types::AnyValue*>(NULL) != __config_cache); // Is initalised
  ...
  for (ccs::types::uint32 index = 0u; (status && (index < __assoc.size())); index += 1u)
    {
      if (strcmp(std::get<1>(__assoc[index]).c_str(), name) == 0)
	{
	  status = ccs::HelperTools::HasAttribute(__config_cache, std::get<0>(__assoc[index]).c_str());
	  ...
	  if (status)
	    {
	      // Update cache
	      log_info("RPCServiceImpl::HandleCAMonitor - Update cache '%s' attribute", std::get<0>(__assoc[index]).c_str());
	      memcpy(ccs::HelperTools::GetAttributeReference(__config_cache, std::get<0>(__assoc[index]).c_str()), 
...
int main (int argc, char** argv) 
{
  ...
  char service [STRING_MAX_LENGTH] = STRING_UNDEFINED; ccs::HelperTools::SafeStringCopy(service, "rpc@L1-Component", STRING_MAX_LENGTH);
  ...
  RPCServiceImpl server (service); server.Connect("rpc@L0-Component");
